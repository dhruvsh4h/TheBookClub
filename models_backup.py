...
class Book(db.Model):
    ...
    genre = db.Column(db.String(50))  # Optional genre field
    rating = db.Column(db.Integer)  # Optional rating 1-5
    review = db.Column(db.Text)  # Optional review
    ...
    def calculate_points(self):
        ...
        if self.status == 'Read':
            ...
            base_points = max(5, self.page_count // 10)
            
            # Bonus points for longer books
            if self.page_count >= 500:
                base_points += 10  # Epic book bonus
            elif self.page_count >= 300:
                base_points += 5   # Long book bonus
